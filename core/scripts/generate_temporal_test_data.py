#!/usr/bin/env python3
"""
Generate realistic temporal test data for Kronos visualization.

This script creates a test object and simulates a week of semantic evolution
with realistic coherence drift and trust decay patterns.

Usage:
    python3 generate_temporal_test_data.py
"""
...
from kronos.temporal_indexer import TemporalIndexer
import time
import json
import uuid
...
def generate_temporal_evolution():
    ...
    storage = CoreStorage()
    kronos = TemporalIndexer(storage)
    
    # Create a test object directly in storage
    test_object = {
        "date": "2025-10-24",
        "description": "DexaFit Denver Financial Summary (Test)",
        "category": "Revenue",
        "amount": 15000.00,
    ...
    # Insert directly into objects table
    with storage.get_connection() as conn:
        cursor = conn.cursor()
        object_id = str(uuid.uuid4())
        now = datetime.now().isoformat()
        cursor.execute("""
            INSERT INTO objects (id, object_type, data, created_at, updated_at)
            VALUES (?, ?, ?, ?, ?)
        """, (object_id, "Transaction", json.dumps(test_object), now, now))
        conn.commit()
...
if __name__ == "__main__":
    print()
    print("=" * 70)
    print("  Kronos Temporal Test Data Generator")
